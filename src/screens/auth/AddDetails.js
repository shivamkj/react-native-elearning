...
const AddDetailsScreen = ({navigation, route: {params}}) => {
  ...
  const onSubmit = () => {
    ...
    if (errors.length == 0) {
      ...
      const {firstName, lastName, password} = formData.current;
      addDetails(params.userId, firstName, lastName, password).then(result => {
        loginUser(
          params.userId,
